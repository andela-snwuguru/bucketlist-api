...
class TestBucketlistItemResources(unittest.TestCase):
    ...
    def register(self):
        ...
        duplicate_response = self.app.post(
            '/api/v1.0/auth/register', data=user)
    ...
    def remove_bucketlist_item(self):
        ...
        item = BucketListItemModel.query.filter_by(
            id=self.bucketlist_item_id).first()
        if item:
            delete(item)
    ...
    def new_bucketlist_item(self):
        ...
        new_bucketlist_item = self.app.post(
            '/api/v1.0/bucketlists/0' + '/items',
        ...
        new_bucketlist_item = self.app.post(
            '/api/v1.0/bucketlists/' + str(self.bucketlist_id) + '/items',
            data={'task': 'buy a private jet'},
            headers={'AccessToken': self.token}
        )
    ...
    def get_single_item(self):
        ...
        get_item = self.app.get(
            '/api/v1.0/bucketlists/' +
            str(self.bucketlist_id) + '/items/' + str(self.bucketlist_item_id),
            headers={'AccessToken': self.token}
        )
    ...
    def update_item(self):
        ...
        put_item = self.app.put(
            '/api/v1.0/bucketlists/0' + '/items/' +
            str(self.bucketlist_item_id),
            data={'done': True},
            headers={'AccessToken': self.token}
        )
    ...
    def delete_bucketlist(self):
        ...
        del_bucketlists = self.app.delete(
            '/api/v1.0/bucketlists/0',
            headers={'AccessToken': self.token}
        )
        self.assertEqual(del_bucketlists.status_code, 404)

        del_bucketlists = self.app.delete(
            '/api/v1.0/bucketlists/' + str(self.bucketlist_id),
            headers={'AccessToken': self.token}
        )
