...
class BucketLists(Resource):
    ...
    def get(self):
        ...
        all_list = BucketListModel.query.filter(BucketListModel.name.like(
            "%" + q + "%"), BucketListModel.created_by.is_(user_id)).all()
        bucketlists = all_list[offset:(limit + offset)]
        result = {'data': [bucketlist.get()
                           for bucketlist in bucketlists], 'page': {}}
...
class BucketListItems(Resource):
    ...
    def get(self, id):
        ...
        if (limit + offset) < len(all_items):
            result['page']['next'] = "/api/v1.0/bucketlists/" + \
                str(id) + "?limit=" + str(limit) + "&page=" + str(next_page)

        if offset:
            result['page']['prev'] = "/api/v1.0/bucketlists" + \
                str(id) + "?limit=" + str(limit) + "&page=" + str(prev_page)
